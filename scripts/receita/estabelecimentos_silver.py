...
class CargaEstabelecimentosSilver:
    ...
    def __init__(self, reference_month: str, minio_client: MinIOClient) -> None:
        self.reference_month = reference_month
        self.raw_prefix = "receita/bronze/estabelecimentos"
        self.silver_prefix = "receita/silver/estabelecimentos"
    ...
    def write_parquet(
        ...
        # Garante uniformidade total de tipos em string antes da gravação final do parquet
        df = df.astype(str)
    ...
    def run(self) -> None:
        ...
        for obj in objects:
            ...
            try:
                ...
                # Filtro: São Paulo (7107) e Ativos (2)
        ...
        if not filtered_dfs:
            print(
                "[WARN] Nenhum registro encontrado para São Paulo (7107) "
                "e Ativos (2)."
            )
            return
        ...
        # Conversão estrita para string e aplicação de schema para evitar quebra de tipagem
        df = df.astype(str)
        ...
        df["numero"] = df["numero"].replace(null_values, pd.NA).fillna("S/N")
        df["complemento"] = (
            df["complemento"]
            .replace(null_values, pd.NA)
        ...
        df["cep"] = df["cep"].replace(null_values, pd.NA).fillna("00000000")
